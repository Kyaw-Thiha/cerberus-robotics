...
from planning.local_planner.core.algorithms import (
...
def test_get_algorithm_unknown_name_raises_with_available_list():
    with pytest.raises(ValueError, match="unknown local planner algorithm 'nope'"):
        get_algorithm("nope")


def test_real_candidates_are_registered():
    names = available_algorithms()
    assert "scan_planner" in names
    assert "vop_nav" in names
    assert get_algorithm("scan_planner").sensor_requirements == {"lidar": "required"}
    assert get_algorithm("vop_nav").sensor_requirements == {"lidar": "required"}
